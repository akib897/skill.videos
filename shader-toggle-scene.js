/* Ported verbatim from ThreeUI src/shaders/skeuomorphic-toggle/shaderToggleGlsl.ts
   and shaderToggleScene.ts (MengTo/threeui@main). TypeScript annotations removed;
   shader source, constants, and frame maths unchanged. */
(function () {
  const SHADER_TOGGLE_VERTEX = `
...
`;
  ...
  function buildShaderToggleFragment(hasDerivatives) {
  ...
  function readAutomaticToggleMode() {
  ...
  function createShaderToggleScene(options) {
    ...
    return {
      ...
      setSpeed(next) { currentSpeed = next; },
      setSize(next) { sizeScale = next; layout(); },
      setOn(next) { target = next ? 1 : 0; },
  ...
  window.ShaderToggleScene = { createShaderToggleScene, readAutomaticToggleMode, clamp };
})();
